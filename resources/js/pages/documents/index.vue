...
const table = useVueTable({
  ...
  onColumnFiltersChange: updaterOrValue => {
    ...
    router.get(
      ...
      {
        preserveState: false, // 🔧 allow Vue to re-render fresh data
        preserveScroll: true, // ✅ optional
        only: ['data'], // 💡 prevent full page reload
      },
...
<template>
  ...
  <Head doc_code="Documents" />

  <AppLayout :breadcrumbs="breadcrumbs">
    <div class="w-full">
      <div class="flex items-center py-4">
        <Input class="max-w-sm" placeholder="Filter doc..."
          :model-value="table.getColumn('doc_code')?.getFilterValue() as string"
          @update:model-value=" table.getColumn('doc_code')?.setFilterValue($event)" />
